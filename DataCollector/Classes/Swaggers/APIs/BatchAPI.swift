...
open class BatchAPI: APIBase {
     ...
     */
    open class func submitBatch(batchRequest: DataCollectorBatchRequest, customerId: String, completion: @escaping ((_ error: Error?) -> Void)) {
        submitBatchWithRequestBuilder(batchRequest: batchRequest, customerId: customerId).execute { (response, error) -> Void in
            completion(error);
        }
    }
     ...
     */
    open class func submitBatchWithRequestBuilder(batchRequest: DataCollectorBatchRequest, customerId: String) -> RequestBuilder<Void> {
        let path = "/v2/batch"
